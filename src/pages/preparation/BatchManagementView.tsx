...
import { Plus, Trash2, Edit2, Check, Layers, Coffee } from 'lucide-react';
...
export function BatchManagementView() {
  ...
  const loadProductsWithBatches = async () => {
    ...
    try {
      ...
      if (productsWithBatches.length > 0 && !selectedProduct) {
        setSelectedProduct(productsWithBatches[0]);
      }
    } catch (error) {
                              ...
                              return (
                                <div key={ing.id} className="text-sm text-secondary-700 flex items-center gap-2">
                                  ...
                                  <span className="text-xs text-secondary-500">
                                    (Available: {material?.quantity || 0} {material?.unit})
                                  </span>
                                </div>
                              );
                            })}
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent"
                    placeholder="How many liters can be made"
                  />
                  <p className="text-xs text-secondary-500 mt-1">
                    Example: "2" if this recipe makes 2 liters of tea
                  </p>
                </div>
